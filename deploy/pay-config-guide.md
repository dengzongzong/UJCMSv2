...
系统提供在线支付的**总开关**（默认关闭），未开启前学员端不显示购买入口，后端也会拦截下单请求。

- **开关位置**：管理后台 → **订单管理** 页面顶部「在线支付」开关。
- **存储**：数据库 `system_setting` 表的 `pay_enabled`（`0`=关闭 `1`=开启），由 `upgrade_all.sql` 幂等写入默认值 `0`。
- **建议流程**：
  1. 按本文档配好微信/支付宝密钥（GitHub Secrets）。
  2. 部署完成后，管理后台打开「在线支付」开关。
  3. 学员端课程详情页出现「立即购买」，支付成功后自动开通。
- **随时停用**：关闭开关即可，已支付订单和开通记录不受影响，重新打开即可恢复。
- **未配密钥就打开开关**：学员下单时会提示"微信/支付宝支付未配置，请联系管理员"。
